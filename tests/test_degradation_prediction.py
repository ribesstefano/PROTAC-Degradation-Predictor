...
def test_active_proba_xgboost():
    ...
    active_prob = get_protac_active_proba(
        ...
        device=device,
        use_xgboost_models=True,
        study_type='similarity',
    )

    print(f'[XGBoost] Active probability: {active_prob} (CPU)')
...
def test_embeddings():
    ...
    protac_emb = get_protac_embedding(
        protac_smiles=protac_smiles,
        e3_ligase=e3_ligase,
        target_uniprot=target_uniprot,
        cell_line=cell_line,
    ...
    print(f'Embeddings: {protac_emb}')
    for model, emb in protac_emb.items():
        print(f'\t- shape: {emb.shape}')

    protac_emb = get_protac_embedding(
        protac_smiles=[protac_smiles] * 8,
        e3_ligase=[e3_ligase] * 8,
        target_uniprot=[target_uniprot] * 8,
        cell_line=[cell_line] * 8,
        device=device,
        study_type='similarity',
    )

    print(f'Embeddings (batched): {protac_emb}')
    for model, emb in protac_emb.items():
        print(f'\t- shape: {emb.shape}')